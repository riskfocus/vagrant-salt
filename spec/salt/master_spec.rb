...
RSpec.describe Salt::Master do
  ...
  context "defaults without syndic" do
    before :each do
      @salt = double
    end
  ...
  context "default with syndics" do
    ...
    describe "#setDefaults" do
      ...
      it "should set defaults" do
        ...
        allow(@salt).to receive(:minion_key=)
        allow(@salt).to receive(:minion_json_config=)
        expect(@salt).to receive(:master_json_config=).with("{\"foo\":\"bar\",\"order_masters\":true}")
        @obj.setDefaults(@salt)
      end
    end
  end
